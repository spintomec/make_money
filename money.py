...
montant_credit_hypothetique = 120000  # Montant de chaque crédit
revenu_locatif_additionnel = 675  # Revenu locatif additionnel par crédit
...
def generer_depense_imprevue():
    # Génère une dépense imprévue aléatoire entre 0 et 5% du revenu total
    return random.uniform(0, 0.05)

capacite_emprunt = calculer_capacite_emprunt(salaire_mensuel, taux_mensuel, duree_emprunt)

for mois in range(135):  # On boucle sur chaque mois jusqu'à avril 2036 (35 ans)
    mois_courant += 1
    if mois_courant > 12:
        mois_courant = 1
        annee_courante += 1
