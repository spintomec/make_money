...
def calculer_valeur_totale_biens(liste_credit_montant, taux_croissance_bien, annee_courante, mois_courant):
    ...
    return sum(liste_credit_montant) * (1 + (taux_croissance_bien/12)) ** (annee_courante - 2024 + mois_courant / 12)
...
def effectuer_simulation(salaire_mensuel, taux_mensuel, duree_emprunt, rentabilite_net, enveloppe_travaux, epargne, epargne_mensuelle, parametres, liste_mois):
    ...
    taux_augmentation_salaire = parametres["taux_augmentation_salaire"]
    ...
    annee_courante = 2024
    mois_courant = 3
    nb_credit = 0 # Compteur de crédit
    delais_credit_counter = 0 # Délais entre l'achat et l'obtention des loyers (pas utilisés pour le moment)
    max_iterations = 30 * 12  # Maximum de 30 ans * 12 mois
    iteration_count = 0  # Compteur d'itérations
    montant_credit_hypothetique = 120000 * 1.08  # Ajout 8% de frais de notaire
    ...
    while iteration_count < max_iterations:  # Limitation du nombre d'itérations
        ...
        if mois_courant > 12:
            mois_courant = 1
            annee_courante += 1
            salaire_mensuel = mise_a_jour_salaire(salaire_mensuel, taux_augmentation_salaire)
        ...
        if delais_credit_counter == 3:
            if capacite_emprunt >= montant_credit_hypothetique:
                ...
                nb_credit += 1
            elif capacite_emprunt + epargne >= montant_credit_hypothetique + enveloppe_travaux:
                epargne += capacite_emprunt - montant_credit_hypothetique - enveloppe_travaux
